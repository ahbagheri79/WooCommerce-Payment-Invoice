...
if (class_exists('WooCommerce')) {
    ...
    if (!$order) {
        ...
        status_header(404);
        wp_send_json_error('Invoice not found.', 404);
        exit;
    }
    ...
    if ($current_user_id !== $order_user_id) {
        // Order does not belong to the current user, return 403 error
        status_header(403);
        wp_send_json_error('Access forbidden.', 403);
        exit;
    }
    ...
    if ($payment_status === 'pending' || $payment_status === 'failed' || $payment_status === 'on-hold') {
        // Order is unpaid, echo "invoice"
        $payment_name = "پیش فاکتور";
    } else {
        // Order is paid, echo "factor"
        $payment_name = "فاکتور";
    }
...
<head>
    ...
    <!-- Site Title -->
    <title> <?php echo $payment_name; ?> سفارش <?php echo $order_id; ?></title>
    <?php wp_head(); ?>
    <style>
        *,
        ::after,
            ...
            <div class="tm_invoice_in">
                <div class="tm_flex tm_flex_column_sm tm_justify_between tm_align_center tm_align_start_sm tm_f14 tm_white_color tm_accent_bg tm_medium tm_padd_8_20 tm_mb25">
                    ...
                    if ($order_created_date) {
                        ...
                        $formatted_created_date = $order_created_date->date('j F Y');

                        // Output the formatted created date in HTML format
                        echo "<p class=\"tm_m0\">تاریخ: $formatted_created_date</p>";
                    } else {
                        // Handle case where creation date is not available
                        echo "<p class=\"tm_m0\">تاریخ فاکتور ثبت نشده است</p>";
                    }
                    ?>
                    <p class="tm_m0 tm_f18 tm_bold"><?php echo $payment_name; ?></p>
                    <?php
                    $serial_number = 'LL' . str_pad($order_id, 6, '0', STR_PAD_LEFT); // Assuming order ID is padded to 6 digits

                    // Output the serial number in HTML format
                    echo "<p class=\"tm_m0\">سریال فاکتور: $serial_number</p>";
                    ?>
                </div>
                <div class="tm_grid_row tm_col_3 tm_padd_20 tm_border tm_accent_border_20 tm_accent_bg_10 tm_mb25 tm_align_center">
                    <div class="tm_border_left tm_accent_border_20 tm_border_none_sm">
                        <div class="tm_logo"><img src="https://invoma.vercel.app/assets/img/logo_accent.svg" alt="Logo">
                        </div>
                    </div>
                    <?php
                    // Get shop address from WooCommerce settings
                    ...
                    // Format the shop address
                    $shop_address_html = "<div class=\"tm_border_left tm_accent_border_20 tm_border_none_sm\">";
                    $shop_address_html .= "$shop_address, $shop_city<br>";
                    $shop_address_html .= "$shop_country $shop_postcode <br>";
                    $shop_address_html .= "</div>";

                    // Output the formatted shop address in HTML format
                    echo $shop_address_html;
                    // Get company details from WooCommerce settings
                    $voip_number = get_option('voip_number');

                    // Check if voip_number is null, empty, or not found
                    if ($voip_number === null || $voip_number === '' || $voip_number === false) {
                        // Set default value
                        $voip_number = "Your default voip number";
                    }
                    $company_email = get_option('admin_email');
                    $company_website = home_url();;

                    // Format the company details
                    $company_details_html = "<div style='direction: ltr'>";
                    $company_details_html .= "021 - $voip_number <br>";
                    $company_details_html .= "$company_email <br>";
                    $company_details_html .= "$company_website";
                    $company_details_html .= "</div>";

                    // Output the formatted company details in HTML format
                    echo $company_details_html;

                    ?>

                </div>
                <div class="tm_padd_20 tm_border tm_accent_border_20 tm_mb25">
                    <p class="tm_primary_color tm_mb2 tm_f16 tm_bold">این فاکتور صادر شده برای:</p>
                    <div class="tm_grid_row tm_col_3 tm_align_center">
                        <?php
                        // Get current user ID
                        $current_user_id = get_current_user_id();

                        // Get customer address details from WooCommerce user profile
                        $customer_country = get_user_meta($current_user_id, 'billing_country', true);
                        $customer_city = get_user_meta($current_user_id, 'billing_city', true);
                        $customer_street = get_user_meta($current_user_id, 'billing_address_1', true);
                        $customer_postcode = get_user_meta($current_user_id, 'billing_postcode', true);

                        // Format the customer address HTML
                        $customer_address_html = "<div class=\"tm_border_left ss tm_accent_border_20 tm_border_none_sm\">";
                        $customer_address_html .= "$customer_country, $customer_city <br>";
                        $customer_address_html .= "$customer_street <br>";
                        $customer_address_html .= "CIF $customer_postcode";
                        $customer_address_html .= "</div>";

                        // Output the formatted customer address in HTML format
                        echo $customer_address_html;

                        ?>
                        <div class="tm_border_left ss tm_accent_border_20 tm_border_none_sm">
                            <?php
                            $current_user_id = get_current_user_id();

                            // Get customer contact details from WooCommerce user profile
                            $customer_email = get_user_meta($current_user_id, 'billing_email', true);
                            $customer_phone = get_user_meta($current_user_id, 'billing_phone', true);
                            $customer_website = get_user_meta($current_user_id, 'billing_website', true);

                            // Concatenate the customer contact details into a single string
                            $customer_contact_html = "$customer_email <br>";
                            $customer_contact_html .= "$customer_phone <br>";
                            $customer_contact_html .= "$customer_website";

                            // Output the customer contact details directly
                            echo $customer_contact_html;
                        ...
                        <div>
                            ...
                            $invoice_details_html .= "تاریخ پرداخت: $order_date <br>";
                            $order_total = number_format($order->get_total(), 0, '', ',') . ' ریال ';
                            $invoice_details_html .= "<b class=\"tm_primary_color\">مجموع پرداختی: $order_total</b> <br>";
                            $invoice_details_html .= "نحوه پرداخت: $order_payment_method";

                            // Output the formatted invoice details in HTML format
                            echo $invoice_details_html;
                            ?>
                        </div>
                    </div>
                </div>
                <div class="tm_table tm_style1 tm_mb40">
                        ...
                        <div class="tm_table_responsive">
                            <table>
                                ...
                                <tr class="tm_accent_bg">
                                    <th class="tm_width_1 tm_semi_bold tm_white_color">ردیف</th>
                                    <th class="tm_width_6 tm_semi_bold tm_white_color">توضیحات</th>
                                    <th class="tm_width_2 tm_semi_bold tm_white_color">قیمت</th>
                                    <th class="tm_width_1 tm_semi_bold tm_white_color">تعداد</th>
                                    <th class="tm_width_2 tm_semi_bold tm_white_color">جمع</th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php
                                ...
                                foreach ($order_items as $item_id => $item) {
                                    // Get product ID for the current item
                                    ...
                                    // Format item prices without any currency symbol

                                    $item_total = number_format($item->get_total(), 0, '', ',') . ' ریال ';
                                    $item_subtotal = number_format($item->get_subtotal(), 0, '', ',') . ' ریال ';

                                    // Add the Iranian Rial (IRR) symbol to the left of the formatted prices

                                    // Get product description
                                    $product_description = $product ? wp_strip_all_tags($product->get_description()) : '';

                                    // Add row to the table HTML
                                    $order_details_html .= "<tr>
                                <td class=\"tm_width_1 tm_accent_border_20\">$item_number</td>
                                <td class=\"tm_width_6 tm_accent_border_20\">
                                   $item_name
                                </td>
                                <td class=\"tm_width_2 tm_accent_border_20\">$item_subtotal</td>
                                <td class=\"tm_width_1 tm_accent_border_20\">$item_quantity</td>
                                <td class=\"tm_width_2 tm_accent_border_20 tm_text_right\">$item_total</td>
                            </tr>";
                    ...
                    <div class="tm_invoice_footer">
                        ...
                        <div class="tm_right_footer">
                            <table>
                                <tbody>
                                <tr class="tm_border_right tm_border_left tm_accent_border_20">
                                    <td class="tm_width_3 tm_primary_color tm_accent_border_20 tm_border_none tm_bold">
                                        جمع اقلام سفارش
                                    </td>
                                    <td class="tm_width_3 tm_primary_color tm_accent_border_20 tm_text_right tm_border_none tm_bold">
                                        <?php
                                        // Get the subtotal of the order
                                        $order_subtotal = $order->get_subtotal();

                                        // Format the subtotal with Iranian Rial (IRR) currency symbol on the left
                                        $formatted_subtotal = number_format($order_subtotal, 0, '', ',') . ' ریال ';

                                        // Output the formatted subtotal
                                        echo $formatted_subtotal;
                                        ?>
                                    </td>
                                </tr>
                                <?php
                                // Loop through each tax item in the order
                                foreach ($order->get_taxes() as $tax_item) {
                                    $tax_label = $tax_item->get_label(); // Get the tax label
                                    $tax_total = $tax_item->get_tax_total(); // Get the tax total

                                    // Format the tax total with Iranian Rial (IRR) currency symbol on the left
                                    $formatted_tax_total = number_format($tax_total, 0, '', ',') . ' ریال ';

                                    // Output a table row for each tax item
                                    echo '<tr class="tm_border_right tm_border_left tm_accent_border_20">';
                                    echo '<td class="tm_width_3 tm_primary_color tm_accent_border_20">' . $tax_label . '</td>';
                                    echo '<td class="tm_width_3 tm_primary_color tm_accent_border_20 tm_text_right">+ ' . $formatted_tax_total . '</td>';
                                    echo '</tr>';
                                }

                                ?>
                                <tr class="tm_border_bottom tm_border_left ss tm_accent_border_20 tm_accent_bg">
                                    <td class="tm_width_3 tm_bold tm_f16 tm_white_color tm_accent_border_20">جمع کل
                                        فاکتور
                                    </td>
                                    <td class="tm_width_3 tm_bold tm_f16 tm_white_color tm_accent_border_20 tm_text_right">
                                        <?php $order_total = $order->get_total();

                                        // Format the total amount with Iranian Rial (IRR) currency symbol on the left
                                        $formatted_total = number_format($order_total, 0, '', ',') . ' ریال ';

                                        // Output the formatted total amount
                        ...
                        <div class="tm_right_footer tm_mobile_hide">
                        </div>
                    </div>
                </div>
                <p class="tm_bold tm_primary_color tm_m0">شرایط و قوانین</p>
                <p class="tm_m0">این فاکتور بصورت خودکار به نام مشتری ایجاد شده است</p>
                <p class="tm_m0">قوانین مربوط به این فاکتور در بخش قوانین سایت درج شده است و استفاده از فاکتور به منزله
                    پذیرش قواین است</p>
            </div>
        </div>
        <div class="tm_invoice_btns tm_hide_print">
          ...
          <span class="tm_btn_icon">
            <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512"><path
                        ...
                        fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="32"/><rect x="128"
                                                                                                           y="240"
                                                                                                           width="256"
                                                                                                           height="208"
                                                                                                           rx="24.32"
                        ...
                        stroke="currentColor" stroke-linejoin="round" stroke-width="32"/><circle cx="392" cy="184"
                                                                                                 r="24"
                                                                                                 fill='currentColor'/></svg>
          </span>
                <span class="tm_btn_text">پرینت فاکتور</span>
            </a>
          ...
          <span class="tm_btn_icon">
            <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512"><path
                ...
                <span class="tm_btn_text">ذخیره</span>
            </button>
        </div>
    </div>
</div>

<script src="<?php echo get_template_directory_uri(); ?>/assets/script/jquery.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/script/jspdf.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/assets/script/html2canvas.min.js"></script>
    ...
    (function ($) {
        ...
        $('#tm_download_btn').on('click', function () {
            ...
            html2canvas(downloadSection[0], { allowTaint: true }).then(function (
                ...
                // Replace external image URLs with the proxy URL
                imgData = imgData.replace(/^data:image\/(png|jpg|jpeg);base64,/, '');
                var proxyUrl = 'path/to/proxy.php?url=' + encodeURIComponent(imgData);
